...
const [date,setDate]= useState(new Date())
  ...
  useEffect(()=>{
   ...
   const handleDate = (date) => {
    ...
    // تأكد أن التاريخ موجود وصالح
    if (!date || !(date instanceof Date) || !isValid(date)) {
        console.error('Invalid date selected:', date);
        setEnabletimeSlotes(false);  // عالج حالة التاريخ غير صالح
        return;
    }

    setDate(date);
    
    const dateFormat = format(date, "EEEE");
  ...
  return (
       ...
       {/* #039b95    */}
          <Calendar
             ...
             onSelect={(d) => {
        if (d) {
            handleDate(d); // فقط إذا كان التاريخ موجود
        } else {
            console.log("No date selected.");
            setEnabletimeSlotes(false); // أو أي منطق آخر عند عدم وجود تاريخ
        }
    }}
