  ...
  useEffect(()=>{
   ...
   const createTimeSlot=(interval)=>{
    ...
    const handleScheduledMeeting= async()=>{
        await setDoc(doc(db, "ScheduledMeeting", Date.now()), {
            meetingName:meetingInfo.meetingName,
            meetingEmail:meetingInfo.email,
            selectedTime:meetingInfo.selectedTime,
            selectedDate:date,
...
<div>
  ...
  { step==1? <DateTimeComponent setSelectedTime={setSelectedTime} enabletimeSlotes={enabletimeSlotes}date={date} handleDate={handleDate} timeSlotes={timeSlotes} />:<UserForm setName={setName} setEmail={setEmail} setNote={setNote}/> }
          ...
          {step==1? <Button onClick={()=>setStep(step+1)} className="float-right px-10 mr-3 hover:bg-[#047772] transition-all text-white cursor-pointer">Next</Button>
          :

          <Button disabled={! name || !email || !note } onClick={()=>setStep(step+1)} className="float-right px-10 mr-3 hover:bg-[#047772] transition-all text-white cursor-pointer">Schedule</Button>
          } 
          </div>
         </div>
