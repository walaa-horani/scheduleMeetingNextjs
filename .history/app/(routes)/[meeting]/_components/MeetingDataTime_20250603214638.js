...
const [date,setDate]= useState(new Date())
  ...
  { step==1? <DateTimeComponent setSelectedTime={setSelectedTime} enabletimeSlotes={enabletimeSlotes}date={date} handleDate={handleDate} timeSlotes={timeSlotes} />:<UserForm/> }
          ...
          <div className='mb-20 '>
          {step==2&&  <Button onClick={()=> setStep(1)} className="float-right px-10 text-white">Back</Button>}
            <Button onClick={()=>setStep(step+1)} className="float-right px-10 mr-3 text-white">{step==1?"Next": "Schedule"}</Button>
          </div>
         </div>
