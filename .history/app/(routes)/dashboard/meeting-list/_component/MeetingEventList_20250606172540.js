...
function MeetingEventList({searchTerm,setSearchTerm}) {
...
const onCopyHandle=(event)=>{
    ...
    const meetingURL = process.env.NEXT_PUBLIC_BASE_URL+"/"+meetingList.meetingName+"/"+event.date
    navigator.clipboard.writeText(meetingURL) 
                toast("Link Copied")
}

const filterdEventList = eventList.filter(event => event.meetingName?.toLowerCase().includes(searchTerm.toLowerCase()))


  return (
    <div className='mt-10 grid grid-col-1 md:grid-cols-2 lg:grid-cols-3'>

       {filterdEventList.length>0?filterdEventList?.map((event)=>(
