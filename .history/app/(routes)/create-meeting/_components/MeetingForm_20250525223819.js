    ...
    <DropdownMenuItem onClick={(e)=> setDuration(60)}>60 Min</DropdownMenuItem>
                ...
                <div>
                   <h2>Choose Program*</h2>    
                    ...
                    {porgrams.map((item)=>(
                        <div onClick={()=> setProgramsURL(item.name)} className={ `cursor-pointer hover:bg-[#e8f7f6] border-slate-200 p-2 flex flex-col justify-center border items-center ${program==item.name&&"bg-[#e8f7f6]"}`}>
                ...
                <Input  onChange={(e)=> setProgram(e.target.value)} placeholder="url"/></> }  
